...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['new_status'])) {
        ...
        $stmt = $pdo->prepare("UPDATE claims SET statut = ? WHERE id = ?");
        ...
        $stmt->execute([$claim_id, $user_id, $msg]);
        
        redirect("traitement.php?id=$claim_id");
    }
            ...
            <div class="col-lg-3">
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        <form method="POST" action="traitement.php?id=<?php echo $claim_id; ?>">
                            <label class="form-label fw-bold small">Changer le statut</label>
                            <select class="form-select mb-3" name="new_status">
                                <option value="en_cours" <?php echo $claim['statut'] == 'en_cours' ? 'selected' : ''; ?>>En cours</option>
                                <option value="attente_info" <?php echo $claim['statut'] == 'attente_info' ? 'selected' : ''; ?>>Attente d'info</option>
                                <option value="traite" <?php echo $claim['statut'] == 'traite' ? 'selected' : ''; ?>>Traité</option>
                                <option value="ferme" <?php echo $claim['statut'] == 'ferme' ? 'selected' : ''; ?>>Fermé</option>
                            </select>
                ...
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                                <div class="fw-bold"><?php echo htmlspecialchars($claim['user_name']); ?></div>
                                <div class="small text-muted"><?php echo htmlspecialchars($claim['user_email']); ?></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-header bg-white p-4 border-bottom d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold">
                            Réclamation #<?php echo $claim['id']; ?> : <?php echo htmlspecialchars($claim['sujet']); ?>
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-4">
                            <span class="badge bg-light text-dark border me-2"><?php echo htmlspecialchars($claim['categorie_nom']); ?></span>
                            <span class="text-muted small"><i class="bi bi-clock me-1"></i> <?php echo format_date($claim['created_at']); ?></span>
                        </div>
